...
namespace lua_tinker
{
	...
	{ 
		...
		{ 
			...
			//loki: push table of T on the top of stack
			push_meta(L, class_name<typename class_type<T>::type>::name());
			//loki: 把这个T类型的table设置为用户数据的metatable
	...
	//loki: 绑定一个c/C++函数到lua中的操作，
	//func 是函数指针
	//functor是一个代理,符合lua函数的调用接口
	// name = functor(func)
	// global function
	template<typename F> 
	...
	{
		//lua_pushstring(L, name);
		//loki: push a number or a object
		push(L, object);
		//lua_settable(L, LUA_GLOBALSINDEX);
		lua_setglobal(L, name);
	}
	...
	RVal call(lua_State* L, const char* name)
	{
		...
		//loki: get function from lua, and push it on the top of stack, pop name
		//lua_pushstring(L, name);
		//lua_gettable(L, LUA_GLOBALSINDEX);
		lua_getglobal(L, name);
	...
	{ 
		...
		if(lua_istable(L, -1))
		{
			...
			//loki: 这两行创建一个c closure, userdata 作为闭包的参数,在userdata中创建一个成员函数指针
			new(lua_newuserdata(L,sizeof(F))) F(func);
			push_functor(L, func);

			//loki: Table.name = function(func)
			lua_rawset(L, -3);
		}
		lua_pop(L, 1);
	}
