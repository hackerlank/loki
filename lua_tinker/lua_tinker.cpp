...
{
	char temp[64];
	sprintf(temp, "%I64lld", *(long long*)lua_topointer(L, 1));
...
{
	char temp[64];
	sprintf(temp, "%I64llu", *(unsigned long long*)lua_topointer(L, 1));
	lua_pushstring(L, temp);
	return 1;
}
...
void lua_tinker::dofile(lua_State *L, const char *filename)
{
	...
	//loki: pop error function 
	lua_remove(L, errfunc);
	//loki: pop lua_pcall 1 result
	lua_pop(L, 1);
}
...
{
	//loki: get t.x, t 是 userdata
	//loki:将table和缺少的域作为参数调用这个函数
	//loki: 把给定索引指向的值的元表压栈
	//loki:
	//int top = lua_gettop(L);
	//print_error(L, "meta_get top = %d", top);
	//loki: push t.meta
	lua_getmetatable(L,1);
	//loki: push key x
	lua_pushvalue(L,2);

	//loki: 注意跟lua_gettable 的区别
	//u2 = t.meta[x]; u2 是 userdata; push u2
	lua_rawget(L,-2);
	...
	//now the size of stack is 3,two para and one result
